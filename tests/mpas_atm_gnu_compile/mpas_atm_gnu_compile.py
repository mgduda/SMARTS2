...
class mpas_atm_gnu_compile:
    ...
    def run(self, env, result, src_dir, test_dir, hpc=None, *args, **kwargs):
        ...
        intel_modset_name = env.list_modsets(name="GNU")
        if not intel_modset_name:
            result.result = "FAILED"
            result.msg = "This env file had no modsets with the name: 'GNU'"
            return result.result

        if not env.load_modset(intel_modset_name[0]):
            result.result = "FAILED"
            result.msg = "Could not load the GNU modset"
            return result.result
