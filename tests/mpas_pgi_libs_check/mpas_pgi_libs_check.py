...
class mpas_pgi_libs_check:
    test_name = "MPAS PGILibs Check"
    ...
    def run(self, env, result, srcDir, testDir, hpc=None, *args, **kwargs):
        ...
        # Load any PGI modset
        modset = env.list_modsets(name="PGI")
        if len(modset) == 0:
            result.result = "FAILED"
            result.msg = "This environment does not have an PGI modset"
            return -1

        if not env.load_modset(modset[0]):
            result.result = "FAILED"
            result.msg = "Could not load the PGI modset"
            return -1

        # List the directories that are in my test directory (I'm looking for `test_files`)
        if 'test_files' in os.listdir(os.path.join(testDir, 'mpas_pgi_libs_check')):
            test_files_dir = os.path.join(testDir, 'mpas_pgi_libs_check', 'test_files')
            print("Location of test files is: ", test_files_dir)
        else:
            print("MPAS_PGI_LIBS_CHECK: Could not find the test_files directory!")
            result.result = "FAILED"
            result.msg = "MPAS_PGI_LIBS_CHECK: Could not find the test_files directory!"
            return -1
        ...
        if os.system('mpicc -o c_mpi mpi.c') != 0:
            ...
            print("MPAS_PGI_LIBS_CHECK: Can compile C with mpicc!")
        ...
        if os.system('mpif90 -o f_mpi f_mpi.f90') != 0:
            ...
            print("MPAS_PGI_LIBS_CHECK: Can compile Fortran with mpif90!")
        ...
        if os.system('mpicc -o c_pnetcdf pnetcdf.c -I$PNETCDF/include -L$PNETCDF/lib -lpnetcdf'):
            ...
            print("MPAS_PGI_LIBS_CHECK: Can compile a C PNetCDF Program!")
        ...
        if os.system('mpif90 -c pnetcdf.f90 -I$PNETCDF/include -L$PNETCDF/lib -lpnetcdf'):
            ...
            print("MPAS_PGI_LIBS_CHECK: Can compile a Fortran PNetCDF (pnetcdf.f90) Program!")
        ...
        if os.system('mpicc -o c_netcdf netcdf.c -I$NETCDF/include -I$PNETCDF/include -L$NETCDF/lib -L$PNETCDF/lib -lnetcdf -lpnetcdf -lhdf5_hl -lhdf5 -ldl -lz'):
            ...
            print("MPAS_PGI_LIBS_CHECK: Can compile a C NetCDF Program!")
        ...
        if os.system('mpif90 -c netcdf.f90 -I$NETCDF/include -I$PNETCDF/include -L$NETCDF/lib -L$PNETCDF/lib -lnetcdf -lpnetcdf -lhdf5_hl -lhdf5 -ldl -lz -lm'):
            ...
            print("MPAS_PGI_LIBS_CHECK: Can compile a Fortran NetCDF Program!")
      ...
      #      return -1
      #  else:
      #      print("MPAS_PGI_LIBS_CHECK: Can compile a C PIO Program!")
        ...
        if os.system('mpif90 -o piof piof.f90 -I$PIO/include -I$NETCDF/include -I$PNETCDF/include -L$PIO/lib -L$NETCDF/lib -L$PNETCDF/lib -lpiof -lpioc -lgptl -lnetcdf -lpnetcdf -lhdf5_hl -lhdf5 -ldl -lz -lm'):
            ...
            print("MPAS_PGI_LIBS_CHECK: Can compile a Fortran PIO Program!")
